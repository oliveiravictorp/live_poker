defmodule LivePokerWeb.UserSettingsLive do
  ...
  def render(assigns) do
    ...
    <div class="mx-auto max-w-sm">
      <.header class="text-center">
        Account Settings
        <:subtitle>Manage your name, email or password</:subtitle>
      </.header>

      <div class="space-y-12 divide-y">
        <div>
          <.simple_form for={@name_form} id="name_form" phx-submit="update_name">
            <.input field={@name_form[:name]} type="text" label="Your name" required />
            <:actions>
  ...
  def handle_event("update_name", params, socket) do
    ...
    case Accounts.apply_user_name(user, user_params) do
      {:ok, user} ->
        name_form =
          user
        ...
        {:noreply, socket |> put_flash(:info, info) |> assign(name_form: name_form)}
